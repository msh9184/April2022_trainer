    ...
    def __init__(self, inplanes, planes, kernel_size=None, kernel_sizes=[5,7], dilation=None, scale = 8, group=1):
        ...
        for i in range(self.nums):
            ...
            for k in kernel_sizes:
                convs += [nn.Sequential(OrderedDict([
...
class SKAttentionModule(nn.Module):
    ...
    def forward(self, x, convs):
        ...
        for conv in convs:
            conv_outs += [conv(x)]
        feats = torch.stack(conv_outs,0)                    # Split: K, B, C, T
        U = sum(conv_outs)                                  # Fuse: B, C, T
        ...
        for fc in self.fcs:                                 # Calculate attention weight
            weight  = fc(Z)
            weights += [(weight.view(bs, c, 1))]            # B, C, 1
        attention_weights = torch.stack(weights, 0)         # K, B, C, 1
        attention_weights = self.softmax(attention_weights) # K, B, C, 1
...
def MainModel(eca_c=1024, eca_s=8, log_input=True, num_mels=80, num_out=192, **kwargs):
    model = SKA_TDNN_msSKA(block=Bottle2neck, C=1024, model_scale=eca_s, log_input=log_input, num_mels=num_mels, num_out=num_out, resblock=ResBlock, **kwargs)
    return model
